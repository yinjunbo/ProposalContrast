...
class SSLHead(nn.Module):
    ...
    def __init__(self, temperature=0.1, tasks=[]):
        ...
        self.criterion_pred = nn.CosineSimilarity(dim=1)
        self.queue = None
    ...
    def forward(self, embeddings):
        ...
        if 'instance' in embeddings.keys():
            pos, neg = embeddings['instance']
            N = pos.size(0)
            logits = torch.cat((pos, neg), dim=1)
            logits /= self.temperature
            labels = torch.zeros((N,), dtype=torch.long).cuda()
            instance_loss = self.criterion(logits, labels)
        ...
        if 'class' in embeddings.keys():
            ...
            bs = output.size(0)//2
            loss = 0
            for i, view_id in enumerate([0, 1]):
                with torch.no_grad():
                    out = output[bs * view_id: bs * (view_id + 1)].detach()
                    q = self.distributed_sinkhorn(out)[-bs:]  # [8192,128]

                subloss = 0
                for v in np.delete(np.arange(2), view_id):
                    x = output[bs * v: bs * (v + 1)] / self.temperature
                    subloss -= torch.mean(torch.sum(q * F.log_softmax(x, dim=1), dim=1))

                loss += subloss
            total_loss += loss * 0.005
            losses['cluster_loss'] = loss


        losses['loss'] = total_loss

        if 'instance' in embeddings.keys():
            diff = torch.max(neg, dim=-1)[0] - pos.squeeze(dim=-1)
            losses['accuracy'] = (diff < 0).sum() * 100.0 / diff.shape[0]
