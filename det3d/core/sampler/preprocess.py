...
def global_rotation_ssl(gt_boxes, points, rotation=np.pi / 4):
    if not isinstance(rotation, list):
        rotation = [-rotation, rotation]
    noise_rotation = np.random.uniform(rotation[0], rotation[1])
    if points is not None:
...
def random_flip_both(gt_boxes, points, probability=0.5, flip_coor=None, kitti_format=False):
    # x flip 
    enable = np.random.choice(
        [False, True], replace=False, p=[1 - probability, probability]
    )
    ...
    if enable and not kitti_format:
...
def random_flip_both_ssl(gt_boxes, points, probability=0.5, flip_coor=None, kitti_format=False):
    param = ""
    # x flip
    enable = np.random.choice(
        [False, True], replace=False, p=[1 - probability, probability]
...
def global_scaling_v2_ssl(gt_boxes, points, min_scale=0.95, max_scale=1.05):
    noise_scale = np.random.uniform(min_scale, max_scale)
    if points is not None:
        points[:, :3] *= noise_scale
    if gt_boxes is not None:
        gt_boxes[:, :-1] *= noise_scale
    return noise_scale, points
...
def intensity_scaling_v2(gt_boxes, points, min_scale=0.95, max_scale=1.05):
    noise_scale1 = np.random.uniform(min_scale, max_scale)
    noise_scale2 = np.random.uniform(min_scale, max_scale)
    points[:, 3] *= noise_scale1
    points[:, 4] *= noise_scale2
    return gt_boxes, points
