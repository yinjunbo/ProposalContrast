...
class Reformat(object):
    ...
    def __call__(self, res, info):
        ...
        correspondence = res["lidar"]["correspondence"] if 'correspondence' in res["lidar"] else None
        correspondence_aug = res["lidar"]["correspondence_aug"] if 'correspondence_aug' in res["lidar"] else None
        voxels = res["lidar"]["voxels"]

        augmentation_list = res["augmentation_list"] if 'augmentation_list' in res else None
        data_bundle = dict(
            metadata=meta,
            points=points,
            ...
            correspondence_aug=correspondence_aug,
            overlap=res["lidar"]["overlap"] if 'overlap' in res["lidar"] else None,
            points_overlap_org=res["lidar"]["points_overlap_org"] if 'points_overlap_org' in res["lidar"] else None,
        )

        if res["mode"] == "train" and 'targets' in res["lidar"]:
