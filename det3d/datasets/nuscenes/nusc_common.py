...
def _fill_trainval_infos(nusc, train_scenes, val_scenes, test=False, interval=1, nsweeps=10, filter_zero=True):
    ...
    for idx,sample in enumerate(tqdm(nusc.sample)):
        """ Manual save info["sweeps"] """
        # Get reference pose and timestamp
        # ref_chan == "LIDAR_TOP"
        if not sample["scene_token"] in val_scenes:
            if not sample["scene_token"] in train_scenes:
                continue
...
def create_nuscenes_infos(root_path, version="v1.0-trainval", split=None, interval=1, nsweeps=10, filter_zero=True):
    ...
    if version == "v1.0-trainval":
        ...
        # train_scenes = train_scenes[:int(len(train_scenes)*0.2)]
            if split=='odd':
                train_scenes = train_scenes[1::2][::interval]
            elif split=='even':
                train_scenes = train_scenes[::2][::interval]
    ...
    train_nusc_infos, val_nusc_infos = _fill_trainval_infos(
        nusc, train_scenes, val_scenes, test, interval=interval, nsweeps=nsweeps, filter_zero=filter_zero
    )
