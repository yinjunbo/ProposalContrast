...
class NuScenesDataset(PointCloudDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(NuScenesDataset, self).__init__(
            root_path, info_path, ssl_mode, pipeline, test_mode=test_mode, class_names=class_names
        )
    ...
    def load_infos(self, info_path):
        ...
        if isinstance(_nusc_infos_all[0], list):
            if len(_nusc_infos_all[0]) == 1:
                _nusc_infos_all = [ele[0] for ele in _nusc_infos_all]
            elif len(_nusc_infos_all[0]) > 1:  # select frames to be used
                if self.ssl_mode:
                    _nusc_infos_all = [ele[len(ele) // 2] for ele in _nusc_infos_all]
        ...
        if not self.test_mode:  # if training
            ...
            # ratios = [0.5 for _ in range(10)] # debug
            ...
            for cls_infos, ratio in zip(list(_cls_infos.values()), ratios):  # todo: replace=False
                ...
                # ).tolist()
                self._nusc_infos += [cls_infos[i] for i in np.random.randint(low=0, high=len(cls_infos), size=int(len(cls_infos) * ratio))]
