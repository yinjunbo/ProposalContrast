...
def convert(idx):
    ...
    for frame_id, data in enumerate(dataset):
        ...
        if not os.path.isfile(os.path.join(LIDAR_PATH, 'seq_{}_frame_{}.pkl'.format(idx, frame_id))):
...
def main(args):
    global fnames 
    fnames = list(glob.glob(args.tfrecord_path))

    print("Number of files {}".format(len(fnames)))

    with Pool(48) as p: # change according to your cpu capacity
