...
def _fill_infos(root_path, frames, version=None, interval=1, split='train', nsweeps=1):
    # load all train infos
    infos = []
    if version == 'convert_info_to_pcdet':
        _info_path = f"data/Waymo/infos_{split}_01sweeps_filter_zero_gt.pkl"
        with open(_info_path, "rb") as f:
            _waymo_infos_all = pickle.load(f)

        with open(f'data/Waymo/openpcdet/openpcdet_{split}_scene_list.txt', 'r') as f:
            scene_list = [line.strip('\n') for line in f.readlines()]

        with open(f'data/Waymo/openpcdet/openpcdet_{split}_scene_mapping.txt', 'r') as f:
            mapping_list = [line.strip('\n') for line in f.readlines()]

        for i, scene_name in enumerate(tqdm(scene_list)):
            mapping_list_key = [ele.split(':')[0] for ele in mapping_list]
            if scene_name in mapping_list_key:
                index = mapping_list_key.index(scene_name)
                infos.append(_waymo_infos_all[index])

    else:
        for idx, frame_name in enumerate(tqdm(frames)):  # global id
            ...
            if interval>1 and split == 'train' and not (idx % interval == 0):
                continue

            pcdet_token = {'context_name':scene_name,
                           'timestamp_micros':int(ref_obj['frame_name'].split("_")[-1])}
            ...
            if split != 'test':
                ...
                # filter boxes without lidar points, only for Waymo-v1.2
...
def get_available_frames(root, split):
    dir_path = os.path.join(root, 'train', 'lidar') if 'train' in split else os.path.join(root, split, 'lidar')
...
def create_waymo_infos(root_path, version=None, interval=1, split='train', nsweeps=1):
    frames = get_available_frames(root_path, split)

    waymo_infos = _fill_infos(
        root_path, frames, version, interval, split, nsweeps
    )
    ...
    if version is None:
        save_name = split
    elif version == 'convert_info_to_pcdet':
        save_name = 'pcdet_' + split
    else:
        save_name = version
    with open(
        os.path.join(root_path, "infos_"+save_name+"_{:02d}sweeps_filter_zero_gt.pkl".format(nsweeps)), "wb"
    ) as f:
