...
def create_groundtruth_database(
    ...
    **kwargs,
    ...
    if dataset_class_name in ["WAYMO", "NUSC"]: 
        if db_path is None:
            if version is None:
                ...
                db_path = root_path / f"gt_database_train_{version}_{nsweeps}sweeps_withvelo"
        if dbinfo_path is None:
            if version is None:
                dbinfo_path = root_path / f"dbinfos_train_{nsweeps}sweeps_withvelo.pkl"
            else:
                dbinfo_path = root_path / f"dbinfos_train_{version}_{nsweeps}sweeps_withvelo.pkl"
    else:
    ...
    for index in tqdm(range(len(dataset))):
        ...
        if dataset_class_name == 'WAYMO' and version is None:
