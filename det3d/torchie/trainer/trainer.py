...
def example_to_device(example, device, non_blocking=False) -> dict:
    ...
    for k, v in example.items():
        ...
        elif k in ['points', 'fps_points', "points_overlap_org", "points_seg_label", "points_moco_seg_label"]:
            if isinstance(v, list):
                points = []
                for ele in v:
                    points.append(ele.to(device, non_blocking=non_blocking))
                example_torch[k] = points
        elif k == "calib":
            calib = {}
...
class Trainer(object):
    ...
    def train(self, data_loader, epoch, **kwargs):
        ...
        for i, data_batch in enumerate(data_loader):
            ...
            # if not 'gt_boxes_and_cls' in data_batch:  # finetune
    ...
    def val(self, data_loader, **kwargs):
        ...
        with open(os.path.join(self.work_dir, "prediction.pkl"), "wb") as f:
