...
def collate_kitti(batch_list, samples_per_gpu=1):
    ...
    if isinstance(example_merged['points'][0], list):
        ret = {}
        for key, elems in example_merged.items():
            if key in ["voxels", "num_points", "num_gt", "voxel_labels", "num_voxels"]:
                ...
                for i, ele in enumerate(elems):
                    elems[i] = torch.tensor(np.concatenate(ele, axis=0))
                ret[key] = torch.tensor(np.concatenate(elems, axis=0))
            elif key in [
                "gt_boxes",
            ]:
                ...
                ret[key] = res
            elif key in ["metadata", "augmentation"]:
                ret[key] = elems
            elif key == "calib":
                ret[key] = {}
                for elem in elems:
                ...
                for i in range(batch_size):#len(all_batch_list[0]['num_voxels'])):  # add the batch index in the front of tensor
                    for j, coor in enumerate(elems[i]):
                        coor_pad = np.pad(
                            coor, ((0, 0), (1, 0)), mode="constant", constant_values=i*len(elems[0])+j  # i:batch, j:frame
                        )
                ...
                if elems is not None:
                    ...
                    for i in range(batch_size):
                        for ele in elems[i]:
                            res.append(torch.tensor(ele))
                    ret[key] = res
            elif key in ["correspondence", "correspondence_aug", "overlap", "points_overlap_org", "points_seg_label", "points_moco_seg_label"]:
                if isinstance(elems, list) and elems[0] is None:
                    continue
                if elems is not None:
                    res = []
                    for i in range(batch_size):
                        res.append(torch.tensor(elems[i]))
                    ret[key] = res
                ...
                res = []
                elems = [x for y in elems for x in y]
                for elem in elems:
                ...
                ret[key] = [np.stack(elem, axis=0) for elem in elems]
            else:
