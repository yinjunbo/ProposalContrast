...
def train_detector(model, dataset, cfg, distributed=False, validate=False, logger=None):
    ...
    if cfg.lr_config.type == "one_cycle":
        ...
        lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[int(total_steps*ratio) for ratio in cfg.lr_config.drop_step], gamma=cfg.lr_config.gamma)
        # lr_scheduler = None
        cfg.lr_config = None 
